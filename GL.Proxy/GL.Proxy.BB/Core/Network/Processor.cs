namespace GL.Proxy.BB.Core.Network
{
    ...
    using GL.Proxy.BB.Core.Network.States;
    using GL.Proxy.BB.Logic.Enums;

    using State = GL.Proxy.BB.Core.Network.States.State;
    ...
    public class Processor : IDisposable
    {
        ...
        private void DataReceived(IAsyncResult AsyncResult)
        {
            try
            {
                ...
                if (Received > 0)
                {
                    ...
                    {
                        Logging.Info(this.GetType(), "Warning, we have nothing to receive again, aborting.");
                    }
                ...
                {
                    Logging.Info(this.GetType(), "Warning, we got disconnected by either the server or the client, aborting.");
                }
            }
        ...
        private void Handle(State State)
        {
            ...
            {
                Logging.Info(this.GetType(), "Warning, we got disconnected by either the server or the client.");
            }
            else
            {
                if (State.Packet.Count >= 7)
                {
                    ...
                    {
                        if      (State.GetType() == typeof(ClientState))    this.ServerSocket.Send(new Packet(State.Packet.Take(MessageLength + 7).ToArray(), Destination.FROM_CLIENT, this.ClientSocket).RebuiltEncrypted);
                        else if (State.GetType() == typeof(ServerState))    this.ClientSocket.Send(new Packet(State.Packet.Take(MessageLength + 7).ToArray(), Destination.FROM_SERVER, this.ClientSocket).RebuiltEncrypted);

                        State.Packet.RemoveRange(0, MessageLength + 7);

                        if (State.Packet.Count > 0)
                        {
                            Logging.Info(this.GetType(), "Warning, we can continue to process the packet, we have some bytes left in the buffer.");
                            ...
                            {
                                Logging.Info(this.GetType(), "Warning, we don't have enough bytes to continue to process the packet.");
                            }
                        }
                        else
                        {
                    ...
                    {
                        Logging.Info(this.GetType(), "Warning, we received a packet, but the buffer length is inferior to the message length.");
                    }
                }
                else
                {
                    Logging.Info(this.GetType(), "Warning, we received a packet, but length is inferior to 7.");
                }
            }
        }
    }
