    ...
    internal class TCPServer
    {
        ...
        internal void OnAcceptCompleted(object Sender, SocketAsyncEventArgs AsyncEvent)
        {
            ...
            else
            {
                ...
                Debug.WriteLine("[*] " + this.GetType().Name + " : " + "Something happened when accepting a new connection, aborting.");
        ...
        internal void Send(Message Message)
        {
            if (Message.Device.Connected)
            {
                ...
                WriteEvent.SetBuffer(Message.ToBytes, Message.Offset, (int) (Message.Length + 5 - Message.Offset));
        ...
        internal void ProcessSend(Message Message, SocketAsyncEventArgs Args)
        {
            if (Args.SocketError == SocketError.Success)
            {
                ...
                if (Message.Length + 5 > Message.Offset)
                {
                    if (Message.Device.Connected)
                    {
                        Args.SetBuffer(Message.Offset, (int)(Message.Length + 5 - Message.Offset));
        ...
        /// <param name="Message">The message.</param>
        /// <param name="Args">The <see cref="SocketAsyncEventArgs"/> instance containing the event data.</param>
        internal void ProcessSend(Device Device, byte[] Array, int Offset, SocketAsyncEventArgs Args)
        {
            if (Args.SocketError == SocketError.Success)
            {
                ...
                if (Array.Length > Offset)
                {
                    if (Device.Connected)
                    {
                        Args.SetBuffer(Offset, (int)(Array.Length - Offset));

                        if (!Device.Socket.SendAsync(Args))
                        {
                            this.ProcessSend(Device, Array, Offset, Args);
                        }
                    }
                    else
                    {
