namespace GL.Servers.BS.Logic
{
    ...
    internal class Timer
    {
            ...
            get
            {
                if (this.TotalSeconds > 0)
                {
                    if (this.StartSubTick != -1)
                    {
                        ...
                        {
                            return Math.Max(
                                (int)(uint)((((-2004318071L * (Ticks + 19) >> 32) + Ticks + 19) >> 31)
                                              + ((((-2004318071L * (Ticks + 19)) >> 32) + Ticks + 19) >> 5)),
                                1);
                        }
        ...
        internal void AdjustSubTick()
        {
            if (this.Started)
            {
                this.TotalSeconds = (Time.GetSecondsInTicks(this.TotalSeconds) + this.StartSubTick) / 20;
                this.StartSubTick = 0;
            }
        }
    }
