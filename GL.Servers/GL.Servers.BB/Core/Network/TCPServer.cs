namespace GL.Servers.BB.Core.Network
{
    ...
    using State = GL.Servers.Logic.Enums.State;
    ...
    internal class TCPServer
    {
        ...
        internal void ProcessAccept(SocketAsyncEventArgs AsyncEvent)
        {
            ...
            Logging.Info(this.GetType(), "New connection from " + Socket.RemoteEndPoint + ".");
            ...
            if (Socket.Connected)
            {
                ...
                Device.State    = DeviceState.Session;
        ...
        internal void Disconnect(SocketAsyncEventArgs AsyncEvent)
        {
            ...
            if (Token.Device.GameMode?.Level?.Player != null)
            {
                Resources.Players.Remove(Token.Device.GameMode.Level.Player);
            }

            Token.Device.Socket.Close();

            Token.Device.State = DeviceState.Disconnected;
