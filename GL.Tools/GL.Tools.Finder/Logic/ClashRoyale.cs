namespace GL.Tools.Finder.Logic
{
    ...
    internal class ClashRoyale : Supercell
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ClashRoyale"/> class.
        /// </summary>
        internal ClashRoyale()
        {
            Console.WriteLine("[*] Clash Royale :");
        }
        ...
        internal ClashRoyale(long Offset) : this()
        {
            this.Offset = Offset;
        }
        ...
        public void Patch(byte[] File)
        {
            if (Console.ReadKey(false).Key == ConsoleKey.Y)
            {
                ...
                using (FileStream FStream = System.IO.File.Create(Directory.GetCurrentDirectory() + "\\Patched\\" + "CR-libg.so", Patched.Count, FileOptions.None))
                {
