    ...
    internal class Gateway
    {
        ...
        internal void Send(Message Message)
        {
            if (Message != null)
            {
                ...
                Logging.Info(this.GetType(), "We are sending the following message : ID " + Message.Identifier + ", Length " + Message.Length + ", Version " + Message.Version + ".");
                ...
                {
                    Logging.Error(this.GetType(), "Warning, aborting the send process because we are disconnected !");
                }
            }
            else
            {
        ...
        private void SendCallback(IAsyncResult AsyncResult)
        {
            if (AsyncResult.AsyncState is Message Message)
            {
                if (!this.Device.Token.Aborting)
                {
                    ...
                    {
                        Logging.Error(this.GetType(), "Warning, we still have bytes to send !");
                    }
                }
            }
            else
